...
- **Управление разрешением экрана:** Возможность задавать разрешение виртуального экрана через переменные окружения (`SCREEN_RESOLUTION`) для предсказуемого вида тестов.
...
- **Управление ресурсами:** Решена фундаментальная проблема падения Chrome путем увеличения разделяемой памяти (`shm_size: 2gb`) и передачи критически важных аргументов браузера (`--no-sandbox`, `--disable-dev-shm-usage`).
- **Надежное сетевое взаимодействие:** Реализован механизм получения прямого IP-адреса контейнера в Docker-сети для обхода проблем с Docker DNS и нестабильным `RestClient`, что гарантирует стабильное соединение.
- **Отказоустойчивость:** Система устойчива к ошибке `304 Not Modified` при повторном запуске/остановке контейнеров.
